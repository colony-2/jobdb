...
func (r *Runtime) preflightScheduleLease(ctx context.Context, lease *runtimeLease) (bool, error) {
	...
	if record == nil {
		r.engine.mu.Unlock()
		return false, swf.ErrJobNotFound
	}
	metadata := cloneJSON(record.metadata)
	chapterCount := int64(len(r.engine.runtimeChapters[lease.jobKey]))
	r.engine.mu.Unlock()
	occ, hasSchedule, err := swf.ExtractScheduleOccurrenceMetadata(metadata)
	if err != nil {
		return false, err
	}
	if !hasSchedule {
	...
	info, err := r.GetSchedule(ctx, swf.ScheduleKey{TenantId: lease.jobKey.TenantId, ScheduleId: occ.ScheduleId})
	if err != nil {
	...
	case swf.ScheduleStateArchived:
	...
	if occ.PreviousJobId != "" {
		failureBits = swf.AppendScheduleFailureBit(failureBits, r.previousJobSucceeded(lease.jobKey.TenantId, occ.PreviousJobId), windowSize)
	}
	if swf.ScheduleFailurePolicyViolated(failureBits, info.FailurePolicy) {
	...
	nextFireAt, err := swf.NextScheduleFire(info.Trigger, time.Now().UTC())
	if err != nil {
		return false, err
	}
	if nextFireAt != nil {
...
func (r *Runtime) previousJobSucceeded(tenantID string, jobID string) bool {
	r.engine.mu.Lock()
	record := r.engine.jobRecords[swf.JobKey{TenantId: tenantID, JobId: jobID}]
	r.engine.mu.Unlock()
	if record == nil {
		return false
	}
	...
	return record.status == swf.JobStatusCompleted && !record.cancelled
}
