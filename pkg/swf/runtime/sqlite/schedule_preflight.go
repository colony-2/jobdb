...
func (r *Runtime) preflightScheduleLease(ctx context.Context, lease *executionLease, metadata json.RawMessage) (bool, error) {
	...
	row, found, err := r.loadScheduleRow(ctx, swf.ScheduleKey{TenantId: lease.jobKey.TenantId, ScheduleId: occ.ScheduleId})
	if err != nil {
		return false, err
	}
	if !found {
	...
	if occ.PreviousJobId != "" {
		failureBits = swf.AppendScheduleFailureBit(failureBits, r.previousJobSucceeded(ctx, lease.jobKey.TenantId, occ.PreviousJobId), windowSize)
	}
...
func (r *Runtime) previousJobSucceeded(ctx context.Context, tenantID string, jobID string) bool {
	row, err := r.loadJobRow(ctx, swf.JobKey{TenantId: tenantID, JobId: jobID})
	if err != nil {
		return false
	}
