...
func TestArtifactCleanupAcrossEngines(t *testing.T) {
	t.Run("toy", func(t *testing.T) {
		...
		engine, cancel := buildToyEngine(t, func(b *jobdb.EngineBuilder) {
...
func (j *artifactCleanupJob) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
...
func (j *artifactCleanupJob) downloadTaskArtifacts(ctx jobdb.JobContext, taskOutput jobdb.TaskData) ([]jobdb.Artifact, error) {
	if j.strataBaseURL == "" {
...
func (t *artifactCleanupTask) Run(ctx jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
...
func runArtifactCleanupScenario(t *testing.T, ctx context.Context, engine jobdb.Engine, filePaths []string) {
	t.Helper()
	...
	require.NoError(t, jobdb.WaitForJobToComplete(ctx, 30*time.Second, jobKey, engine))
