...
func (customIDJob) Run(_ jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	return data, nil
}
...
func TestReplayAfterExternalTaskCompletionAcrossBuiltInRuntimes(t *testing.T) {
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			...
			replayed, err := built.Engine.ReplayJobRun(ctx, jobdb.ReplayRunRequest{JobKey: jobKey})
			if err != nil {
...
func TestEngineMetadataFilteredWorkersAndManualJobLeaseAcrossBuiltInRuntimes(t *testing.T) {
	...
	ws := jobdbtest.MustWorkSetWithOptions(t, customIDJob{}, jobdb.WorkRegistrationOptions{MetadataFilter: metaFilter})
...
func TestFindTasksWaitingWithMetadataFilterAcrossBuiltInRuntimes(t *testing.T) {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			...
			var handles []jobdb.TaskHandle
			deadline := time.Now().Add(5 * time.Second)
			for time.Now().Before(deadline) {
				handles, err = built.Engine.FindTasksWaiting(ctx, jobdb.FindTasksWaitingRequest{
					JobType:        ws.JobWorker.Name(),
