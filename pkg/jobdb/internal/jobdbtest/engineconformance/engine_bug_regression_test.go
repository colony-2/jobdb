...
func (artifactPassthroughJob) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
...
func (artifactPassthroughTask) Run(_ jobdb.TaskContext, _ jobdb.TaskData) (jobdb.TaskData, error) {
...
func (j failedChildJob) Run(_ jobdb.JobContext, _ jobdb.JobData) (jobdb.JobData, error) {
	if j.started != nil {
		close(j.started)
	}
	<-j.release
...
func (j *awaitFailedChildJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (j *awaitFailedChildViaRunOutputJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (j childRunOutputRetryParentJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (t childRunOutputStartTask) Run(ctx jobdb.TaskContext, data jobdb.TaskData) (jobdb.TaskData, error) {
...
func (t childRunOutputFinishTask) Run(ctx jobdb.TaskContext, data jobdb.TaskData) (jobdb.TaskData, error) {
	if t.engine == nil || *t.engine == nil {
		return nil, errors.New("engine not configured")
	}
...
func (childRunOutputUnexpectedBranchTask) Run(_ jobdb.TaskContext, data jobdb.TaskData) (jobdb.TaskData, error) {
	return data, nil
}
...
func (childRunOutputFailingChildJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (childRunOutputFailTask) Run(_ jobdb.TaskContext, _ jobdb.TaskData) (jobdb.TaskData, error) {
...
func TestArtifactPassthroughAcrossBuiltInRuntimes(t *testing.T) {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			...
			go func() {
				done <- jobdb.WaitForJobToComplete(ctx, 3*time.Second, jobKey, built.Engine)
			}()
...
func TestAwaitFailedChildReplayAcrossBuiltInRuntimes(t *testing.T) {
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			...
			_, replayErr := built.Engine.ReplayJobRun(ctx, jobdb.ReplayRunRequest{JobKey: parentKey})
			if replayErr == nil {
