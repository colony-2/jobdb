...
func (j explicitIDEchoJob) Run(_ jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	return data, nil
}
...
func (j explicitIDBlockingJob) Run(_ jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	if j.started != nil {
		select {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			...
			t.Run("completed", func(t *testing.T) {
				...
				runnable, err := jobdb.GetJobForRun(ctx, built.Runtime, jobdb.GetJobForRunRequest{
					JobKey:    handle.JobKey,
				...
				if outcome.Status != jobdb.JobRunCompleted {
				...
				replayed, err := jobdb.GetJobForRun(ctx, built.Runtime, jobdb.GetJobForRunRequest{
					JobKey:    handle.JobKey,
					JobWorker: jobdbtest.SequenceJob{},
				...
				if cached.Status != jobdb.JobRunCompleted {
			...
			t.Run("failed", func(t *testing.T) {
				...
				if outcome.Status != jobdb.JobRunFailed {
					t.Fatalf("unexpected outcome status %q", outcome.Status)
				}
				if !outcome.LeaseAcquired {
					t.Fatal("expected helper to acquire the lease")
			...
			t.Run("suspended_missing_capability", func(t *testing.T) {
				...
				if outcome.Status != jobdb.JobRunSuspended {
					t.Fatalf("unexpected outcome status %q", outcome.Status)
				}
				if !outcome.LeaseAcquired {
					t.Fatal("expected helper to acquire the lease")
			...
			t.Run("not_leaseable_while_active", func(t *testing.T) {
				...
				if cached.Status != jobdb.JobRunNotLeaseable {
					t.Fatalf("unexpected cached outcome status %q", cached.Status)
				}
				outcome, err := runnable.Run(nil)
				if err != nil {
				...
				if outcome.Status != jobdb.JobRunNotLeaseable {
					t.Fatalf("unexpected outcome status %q", outcome.Status)
				}
				if outcome.LeaseAcquired {
