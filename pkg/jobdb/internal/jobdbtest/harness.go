...
type RuntimeHarness struct {
	...
	New                    func(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness
}
...
func MustWorkSet(t *testing.T, job jobdb.JobWorker, tasks ...jobdb.TaskWorker) jobdb.WorkSet {
	t.Helper()
	ws, err := jobdb.AsWorkSet(job, tasks...)
	if err != nil {
...
func MustWorkSetWithOptions(t *testing.T, job jobdb.JobWorker, opts jobdb.WorkRegistrationOptions, tasks ...jobdb.TaskWorker) jobdb.WorkSet {
	t.Helper()
	ws, err := jobdb.AsWorkSetWithOptions(job, opts, tasks...)
	if err != nil {
		t.Fatalf("build workset: %v", err)
	}
	return *ws
...
func WaitForEngineStatus(t *testing.T, ctx context.Context, engine jobdb.Engine, jobKey jobdb.JobKey, want jobdb.JobStatus) {
	t.Helper()
...
func WaitForTaskHandle(t *testing.T, ctx context.Context, engine jobdb.Engine, jobType string, taskType string, tenantIDs []string) jobdb.TaskHandle {
	t.Helper()
...
func MustStartJobAsync(t *testing.T, engine jobdb.Engine, start jobdb.SubmitJob) <-chan error {
	t.Helper()
...
func (j SequenceJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	var (
...
func (AddOneTask) Run(_ jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
...
func (DoubleTask) Run(_ jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
	n, err := numberFromTaskData(input)
	if err != nil {
		return nil, err
	}
...
func (FailingJob) Run(_ jobdb.JobContext, _ jobdb.JobData) (jobdb.JobData, error) {
...
func newToyHarness(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
	runtime := toyruntime.New()
	return buildHarness(t, "toy", runtime, true, func() {}, workers...)
}

func newDirectHarness(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
...
func newSQLiteHarness(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
...
func newRemoteToyHarness(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
...
func newRemoteDirectHarness(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
	embedded, err := directruntime.StartEmbeddedRuntime(context.Background())
	if err != nil {
		t.Fatalf("start embedded direct runtime: %v", err)
...
func newRemoteSQLiteHarness(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
	embedded, err := sqliteruntime.StartEmbeddedRuntime(context.Background())
	if err != nil {
		t.Fatalf("start embedded sqlite runtime: %v", err)
...
func newExternalRemoteHarness(t *testing.T, workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
...
func buildHarness(t *testing.T, name string, runtime jobdb.WorkflowRuntime, startLoop bool, shutdown func(), workers ...jobdb.WorkSet) *BuiltRuntimeHarness {
	t.Helper()
	builder := jobdb.NewEngineBuilder().WithRuntime(runtime)
	workerTenantID := "tenant-worker-" + name
	if len(workers) > 0 {
		builder.WithWorkerTenantId(workerTenantID)
	}
	for _, ws := range workers {
		tasks := make([]jobdb.TaskWorker, 0, len(ws.TaskWorkers))
