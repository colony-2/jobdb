...
func TestExplicitJobIDParityAcrossBuiltInRuntimes(t *testing.T) {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			compareAcrossModes(t, harness, []jobdb.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) lifecycleObservation {
...
func TestCancelJobParityAcrossBuiltInRuntimes(t *testing.T) {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			compareAcrossModes(t, harness, []jobdb.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) cancelObservation {
				jobKey, err := subject.SubmitJob(ctx, jobdb.SubmitJob{
					TenantId: subject.built.WorkerTenantID,
					JobType:  ws.JobWorker.Name(),
					JobID:    "cancel-parity",
...
func TestRestartValidationParityAcrossBuiltInRuntimes(t *testing.T) {
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name+"/negative-last-step", func(t *testing.T) {
			...
			compareAcrossModes(t, harness, []jobdb.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) errorObservation {
		...
		t.Run(harness.Name+"/retry-boundary", func(t *testing.T) {
			run := func(mode parityMode) errorObservation {
				...
				return observeViaMode(t, harness, mode, []jobdb.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) errorObservation {
					jobKey, err := subject.SubmitJob(ctx, jobdb.SubmitJob{
						TenantId: subject.built.WorkerTenantID,
						JobType:  job.Name(),
...
func TestPrerequisiteParityAcrossBuiltInRuntimes(t *testing.T) {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			compareAcrossModes(t, harness, []jobdb.WorkSet{
