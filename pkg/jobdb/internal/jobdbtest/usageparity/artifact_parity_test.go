...
func TestArtifactStorageOnTaskErrorParityAcrossBuiltInRuntimes(t *testing.T) {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			compareAcrossModes(t, harness, []jobdb.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) artifactErrorObservation {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			run := func(mode parityMode) artifactCleanupObservation {
				...
				return observeViaMode(t, harness, mode, []jobdb.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) artifactCleanupObservation {
					jobKey, err := subject.SubmitJob(ctx, jobdb.SubmitJob{
						TenantId: subject.built.WorkerTenantID,
						JobType:  job.Name(),
