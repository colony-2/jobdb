...
import (
	...
	jobdbtest "github.com/colony-2/jobdb/pkg/jobdb/internal/jobdbtest"
...
func (s scenarioSubject) Engine() jobdb.Engine {
...
func mustReadEngineArtifactBytes(t *testing.T, ctx context.Context, engine jobdb.Engine, tenantID string, key jobdb.ArtifactKey) normalizedArtifact {
	t.Helper()
...
func (j passthroughJob) Run(_ jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	return data, nil
}
...
func (pendingJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (externalApprovalJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (j awaitChildJob) Run(_ jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	if j.started != nil {
		close(j.started)
	}
	<-j.release
...
func (j awaitParentJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (j awaitDurationJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (j *retryJob) Run(_ jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	j.attempts++
	if j.attempts == 1 {
...
func (retryTaskJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (t *retryTask) Run(_ jobdb.TaskContext, data jobdb.TaskData) (jobdb.TaskData, error) {
	t.attempts++
	if t.attempts == 1 {
...
func (j namedFailingJob) Run(_ jobdb.JobContext, _ jobdb.JobData) (jobdb.JobData, error) {
	if j.message == "" {
...
func (j failingTaskJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (t namedFailingTask) Run(_ jobdb.TaskContext, _ jobdb.TaskData) (jobdb.TaskData, error) {
	if t.message == "" {
...
func (j taskErrorWithArtifactJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	return ctx.DoTask(jobdb.RunPolicy{}, j.task, data)
}
...
func (t artifactFailingTask) Run(_ jobdb.TaskContext, _ jobdb.TaskData) (jobdb.TaskData, error) {
	output := t.output
	if output == "" {
...
func (j jobErrorWithArtifact) Run(_ jobdb.JobContext, _ jobdb.JobData) (jobdb.JobData, error) {
	output := j.output
	if output == "" {
...
func (awaitTaskJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (t awaitTaskWorker) Run(ctx jobdb.TaskContext, data jobdb.TaskData) (jobdb.TaskData, error) {
...
func (transformPendingJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (j cleanupArtifactJob) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
...
func (t cleanupArtifactTask) Run(_ jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
...
func (echoTask) Run(_ jobdb.TaskContext, data jobdb.TaskData) (jobdb.TaskData, error) {
	return data, nil
}
...
func (j singleEchoJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	if j.runs != nil {
		*j.runs++
	}
