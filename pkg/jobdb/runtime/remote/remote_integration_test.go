...
func TestRemoteRuntimeSupportsExplicitRestartJobIDs(t *testing.T) {
	...
	builder := jobdb.NewEngineBuilder().WithRuntime(underlying).WithWorkerTenantId(tenantID)
	...
	waitForEngineStatus(t, ctx, engine, originalKey, jobdb.JobStatusCompleted)
...
func (remoteSequenceJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	first, err := ctx.DoTask(jobdb.RunPolicy{}, remoteAddOneTaskName, data)
	if err != nil {
		return nil, err
	}
	second, err := ctx.DoTask(jobdb.RunPolicy{}, remoteDoubleTaskName, first)
	if err != nil {
		return nil, err
	}
	return second, nil
...
func (remoteAddOneTask) Run(_ jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
	...
	return jobdb.NewTaskDataOrPanic(map[string]int{"n": payload["n"] + 1}), nil
}
...
func (remoteDoubleTask) Run(_ jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
	raw, err := input.GetData()
	if err != nil {
		return nil, err
	}
	...
	return jobdb.NewTaskDataOrPanic(map[string]int{"n": payload["n"] * 2}), nil
}
