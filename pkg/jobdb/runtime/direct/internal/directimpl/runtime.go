...
func (r *Runtime) FindTasksWaitingForCapability(ctx context.Context, jobType string, taskType string, tenantIds []string) ([]jobdb.TaskHandle, error) {
	...
	handles := make([]jobdb.TaskHandle, 0, len(jobs))
	for _, j := range jobs {
		...
		handles = append(handles, &taskHandleImpl{
			...
			metadata:      jobdb.AppMetadataFromStoredMetadata(details.Metadata),
...
func (r *Runtime) GetWaitingTask(ctx context.Context, key jobdb.JobKey) (jobdb.TaskHandle, error) {
	...
	return &taskHandleImpl{
		...
		payload:       job.Payload,
		metadata:      jobdb.AppMetadataFromStoredMetadata(job.Metadata),
		inputOrdinal:  tw.InputStep,
		outputOrdinal: tw.OutputStep,
		runtime:       r,
		nextNeed:      pgwf.Capability(tw.Next),
