...
func (prereqSuccessWorker) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
	return input, nil
}
...
func (prereqFailWorker) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
...
func (prereqDependentWorker) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
	return input, nil
}
...
func (prereqRestartJobWorker) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
...
func (prereqRestartTaskWorker) Run(ctx jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
	return input, nil
}
...
func TestPrerequisitesSuccessAndComplete(t *testing.T) {
	...
	engine := buildDirectEngine(t, postgresDSN, baseURL, strata.APIKey, func(b *jobdb.EngineBuilder) {
	...
	for _, key := range []jobdb.JobKey{successKey, failKey, successDependent, failedDependent, completeDependent} {
		if err := jobdb.WaitForJobToComplete(ctx, 30*time.Second, key, engine); err != nil {
...
func TestRestartPrerequisitesCheckedAtRestartExtra(t *testing.T) {
	...
	if err := jobdb.WaitForJobToComplete(ctx, 30*time.Second, baseKey, engine); err != nil {
	...
	if err := jobdb.WaitForJobToComplete(ctx, 30*time.Second, failKey, engine); err != nil {
	...
	if err := jobdb.WaitForJobToComplete(ctx, 30*time.Second, restartKey, engine); err != nil {
