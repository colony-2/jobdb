...
func TestBasicWorkflowIntegration(t *testing.T) {
	...
	engine1 := buildDirectEngine(t, postgresDSN, baseURL, strata.APIKey, func(b *jobdb.EngineBuilder) {
		b.WithLogger(logger).WithWorkerTenantId(tenantID).PlusWorkers(pipeJob{}, addOneTask{})
	})

	engine2 := buildDirectEngine(t, postgresDSN, baseURL, strata.APIKey, func(b *jobdb.EngineBuilder) {
...
func (pipeJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (addOneTask) Run(ctx jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
	n := taskNumber(input)
	if ctx.Logger != nil {
...
func (doubleTask) Run(ctx jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
	n := taskNumber(input)
	if ctx.Logger != nil {
...
func userInputWatcher(ctx context.Context, t *testing.T, engine jobdb.Engine, tenantIDs []string) {
	for {
		select {
		case <-ctx.Done():
			return
