...
func RunWorkflowRuntimeConformance(t *testing.T, harnesses ...Harness) {
	t.Helper()
	if len(harnesses) == 0 {
	...
	t.Run("chapter_and_artifact_round_trip", func(t *testing.T) {
		runChapterAndArtifactRoundTrip(t, harnesses)
	})
	...
	t.Run("poll_work_requires_tenant", func(t *testing.T) {
		runPollWorkRequiresTenant(t, harnesses)
	})
	t.Run("conflict_behavior", func(t *testing.T) {
		runConflictBehavior(t, harnesses)
	})
	t.Run("explicit_job_id_idempotency", func(t *testing.T) {
		runExplicitJobIDIdempotency(t, harnesses)
	})
	t.Run("explicit_job_id_duplicate_submit_state_preserving", func(t *testing.T) {
		runExplicitJobIDDuplicateSubmitStatePreserving(t, harnesses)
	})
	t.Run("explicit_job_id_conflict_classes", func(t *testing.T) {
		runExplicitJobIDConflictClasses(t, harnesses)
	})
	t.Run("explicit_job_id_tenant_scoped", func(t *testing.T) {
		runExplicitJobIDTenantScoped(t, harnesses)
	})
	t.Run("poll_work_metadata_filtering", func(t *testing.T) {
		runPollWorkMetadataFiltering(t, harnesses)
	})
	t.Run("get_job_lease", func(t *testing.T) {
		runGetJobLease(t, harnesses)
	})
	t.Run("get_job_for_run", func(t *testing.T) {
		runGetJobForRun(t, harnesses)
	})
}

func runConstructAndExecuteThroughBuilder(t *testing.T, harnesses []Harness) {
	ws := MustWorkSet(t,
		sequenceJob{Steps: []string{AddOneTaskName, DoubleTaskName}},
		addOneTask{},
		doubleTask{},
	)
	...
	for _, harness := range harnesses {
		...
		t.Run(harness.Name, func(t *testing.T) {
			built := buildFixture(t, harness, ws)
			...
			WaitForEngineStatus(t, ctx, built.Engine, jobKey, jobdb.JobStatusCompleted)

			result, err := jobResultForTest(built.Engine, ctx, jobKey)
			if err != nil {
				t.Fatalf("get job result: %v", err)
			}
			if got := MustDecodeNumberTaskData(t, result); got != 4 {
...
func runWorkflowRuntimeLifecycle(t *testing.T, harnesses []Harness) {
	ws := MustWorkSet(t,
		sequenceJob{Steps: []string{AddOneTaskName, DoubleTaskName}},
		addOneTask{},
		doubleTask{},
...
func runExecutionLeaseSubmitJobTracksParent(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			if !harness.Capabilities.Leases {
...
func assertExecutionLeaseSubmitJobTracksParent(t *testing.T, built *builtFixture) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
...
func runChapterAndArtifactRoundTrip(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			if !harness.Capabilities.RuntimeStorage {
...
func runListChaptersRange(t *testing.T, harnesses []Harness) {
	ws := MustWorkSet(t,
		sequenceJob{Steps: []string{AddOneTaskName, DoubleTaskName}},
		addOneTask{},
		doubleTask{},
...
func runLeaseOperations(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			if !harness.Capabilities.Leases {
...
func runPollWorkRequiresTenant(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			built := buildFixture(t, harness)
...
func runConflictBehavior(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			...
			t.Run("commit_if_waiting_conflicts_after_completion", func(t *testing.T) {
				ws := MustWorkSet(t,
					sequenceJob{Steps: []string{MissingTaskName}},
				)
				built := buildFixture(t, harness, ws)
				defer built.Shutdown(t)
				...
				_ = WaitForTaskHandle(t, ctx, built.Engine, SequenceJobName, MissingTaskName, []string{jobKey.TenantId})
...
func runExplicitJobIDIdempotency(t *testing.T, harnesses []Harness) {
	ws := MustWorkSet(t,
		sequenceJob{Steps: []string{AddOneTaskName, DoubleTaskName}},
		addOneTask{},
		doubleTask{},
	...
	for _, harness := range harnesses {
		...
		t.Run(harness.Name, func(t *testing.T) {
			...
			WaitForEngineStatus(t, ctx, built.Engine, originalKey, jobdb.JobStatusCompleted)
...
func runExplicitJobIDDuplicateSubmitStatePreserving(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			...
			t.Run("active", func(t *testing.T) {
				...
				ws := MustWorkSet(t, explicitIDBlockingJob{
				...
				WaitForRuntimeStatus(t, ctx, built.Runtime, handle.JobKey, jobdb.JobStatusActive)
			...
			t.Run("terminal", func(t *testing.T) {
				ws := MustWorkSet(t, explicitIDEchoJob{name: "explicit-terminal-job"})
				built := buildFixture(t, harness, ws)
				defer built.Shutdown(t)
				...
				if got := MustDecodeNumberTaskData(t, result); got != 9 {
...
func runExplicitJobIDConflictClasses(t *testing.T, harnesses []Harness) {
...
func runExplicitJobIDTenantScoped(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			built := buildFixture(t, harness)
...
func runPollWorkMetadataFiltering(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			built := buildFixture(t, harness)
...
func runGetJobLease(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			built := buildFixture(t, harness)
...
func runGetJobForRun(t *testing.T, harnesses []Harness) {
	for _, harness := range harnesses {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
			...
			t.Run("completed", func(t *testing.T) {
				...
				if got := MustDecodeNumberTaskData(t, outcome.Output); got != 7 {
				...
				if got := MustDecodeNumberTaskData(t, replayedOutcome.Output); got != 7 {
			...
			t.Run("suspended_missing_capability", func(t *testing.T) {
				...
				runnable, err := workflow.GetJobForRun(ctx, built.Runtime, workflow.GetJobForRunRequest{
					JobKey:    handle.JobKey,
					JobWorker: sequenceJob{Steps: []string{MissingTaskName}},
