...
func (w errorTaskWorker) Run(_ jobdb.TaskContext, _ jobdb.TaskData) (jobdb.TaskData, error) {
	return nil, w.err
}
...
func (j singleTaskJob) Run(ctx jobdb.JobContext, input jobdb.JobData) (jobdb.JobData, error) {
...
func (w errorJobWorker) Run(_ jobdb.JobContext, _ jobdb.JobData) (jobdb.JobData, error) {
	return nil, w.err
}

func assertTaskOutcomePayloadKind(t *testing.T, engine jobdb.Engine, jobKey jobdb.JobKey, expected string) {
	t.Helper()
...
func assertJobOutcomePayloadKind(t *testing.T, engine jobdb.Engine, jobKey jobdb.JobKey, expected string) {
	t.Helper()
	run, err := engine.GetJobRun(context.Background(), jobdb.GetJobRunRequest{JobKey: jobKey})
	if err != nil {
		t.Fatalf("get job run: %v", err)
...
func waitForJobResult(t *testing.T, engine jobdb.Engine, dsn string, jobKey jobdb.JobKey, expectAppError, expectSysError bool) (jobdb.TaskData, error) {
	t.Helper()
