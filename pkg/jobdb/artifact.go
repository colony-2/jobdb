...
type Artifact interface {
	...
	// Cleanup is called by jobdb after the artifact has been fully consumed
...
// artifactKeySetter allows jobdb to update the artifact key after persistence.
...
// This is intended for use by jobdb persistence code after the artifact is saved.
...
// Example:
//
//	art := jobdb.NewArtifactFromBytes("config.json", jsonBytes)
...
// Example:
//
//	art := jobdb.NewArtifactFromReader("output.txt", reader, 1024)
...
// The file will be automatically removed when jobdb is done (cleanup).
//
// Example:
//
//	art, _ := jobdb.NewArtifactFromFile("build.tar.gz", "/tmp/build.tar.gz")
...
// Example:
//
//	art, _ := jobdb.NewArtifactFromFileNoCleanup("input.txt", "/data/input.txt")
...
// Example:
//
//	art := jobdb.NewArtifact("custom", func() (io.ReadCloser, int64, error) {
//	    f, _ := os.Open(path)
//	    info, _ := f.Stat()
