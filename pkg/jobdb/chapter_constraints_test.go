...
func TestChapterConstraintsAcrossEngines(t *testing.T) {
	tests := []struct {
		name        string
		setupEngine func(t *testing.T) (jobdb.Engine, func())
		skipReason  string
	}{
		{
			name: "toy",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			t.Run("non-deterministic job fails with determinism error", func(t *testing.T) {
				...
				ws2, err := jobdb.AsWorkSet(&nonDeterministicJob{}, &incrementTask{})
				if err != nil {
...
func (j *deterministicJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
...
func (j *nonDeterministicJob) Run(ctx jobdb.JobContext, data jobdb.JobData) (jobdb.JobData, error) {
	// Execute first task
...
func (t *incrementTask) Run(ctx jobdb.TaskContext, input jobdb.TaskData) (jobdb.TaskData, error) {
...
func waitForJobToComplete(t *testing.T, engine jobdb.Engine, jobKey jobdb.JobKey) {
	t.Helper()
	ctx := context.Background()
